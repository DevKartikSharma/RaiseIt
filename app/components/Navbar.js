  ...
  return <>
    <div className='flex justify-between items-center px-10 max-sm:px-5 h-15 baloo-medium text-2xl bg-[rgba(26,25,44,0.59)] relative select-none'>
      ...
      <div className="navigation flex justify-center items-center text-xl max-sm:text-lg">
        ...
        <Link href={"/signup"}><span className={`rounded-lg hover:text-[rgba(33,21,74)] hover:bg-[rgb(206,206,206)] px-4 max-sm:px-2 py-1 ${(!session?.user?.image) ? '' : 'hidden'} `} >Sign Up</span></Link>
        {
          (session?.user?.image)
          && <button onClick={() => HandleDrop()} className='flex justify-center items-center space-x-2 pr-2'><Image
            ...
            width={40}
            height={40}
            className={`h-10 w-10 bg-gray-700 rounded-full overflow-hidden border-[2px] ${Dropdown ? 'border-white' : 'border-none'}`}
            alt="Profile"
          />
        ...
        {
          (!session?.user?.image)
          && <Link href={"/login"}><span className='rounded-lg hover:text-[rgba(33,21,74)] hover:bg-[rgb(206,206,206)] px-4 max-sm:px-2 py-1'>Login</span></Link>
        }
        {(session?.user?.image) &&
          <div tabIndex={0} onBlur={(e) => {
            if (!e.currentTarget.contains(e.relatedTarget)) { SetDropdown(true) }
          }} className={`flex flex-col items-center absolute top-[125%] right-3 px-1 h-fit w-fit baloo-light text-lg bg-[rgba(93,93,93,0.25)] max-sm:bg-[rgba(93,93,93,0.80)] rounded-lg pt-3 ${(!Dropdown) ? '' : 'hidden'} z-50`}>
